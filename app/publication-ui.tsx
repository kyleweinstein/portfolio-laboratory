...
export function PortfolioDetailView({ portfolio }: { portfolio: PortfolioDetail }) {
  ...
  return <>
    ...
    {portfolio.analytics.optimizedAllocation.length > 0 && <section className="card publication-analytics-card"><span className="eyebrow">Stored scenario</span><h2>Optimized sleeve allocation</h2><p className="chart-intro">This read-only, long-only scenario treats cash as a possible holding. Its 15% cap rises only when needed to make a smaller portfolio fully allocable. It is not a trade instruction.</p><div className="published-allocation-list">{portfolio.analytics.optimizedAllocation.map(item => <div key={item.symbol}><strong>{item.symbol}</strong><span><i style={{ width: `${Math.max(0, Math.min(100, item.weightPercent))}%` }}/></span><b>{formatPercent(item.weightPercent)}</b></div>)}</div></section>}
  </>;
}
